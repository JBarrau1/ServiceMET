...
class AppDatabase {
  ...
  Future<String> generateSessionId(String seca) async {
    ...
    try {
      // Buscar el máximo session_id existente para ese SECA específico
      final result = await db.rawQuery(
          'SELECT MAX(CAST(session_id AS INTEGER)) as max_id '
              ...
              'AND session_id GLOB \"[0-9]*\"', // Solo IDs numéricos
          [seca]
      );
      ...
      // En caso de error, generar sessionId basado en timestamp
  ...
  Future<Map<String, dynamic>?> getRegistroBySeca(String seca, String sessionId) async {
    ...
    final result = await db.query(
      'registros_calibracion',
      where: 'seca = ? AND session_id = ?', // ← BUSCAR POR AMBOS
  ...
  Future<void> upsertRegistroCalibracion(Map<String, dynamic> registro) async {
    ...
    // Verificar si ya existe un registro con el MISMO SECA y MISMO SESSION_ID
    final existing = await db.query(
      'registros_calibracion',
      where: 'seca = ? AND session_id = ?', // ← BUSCAR POR AMBOS CAMPOS
      whereArgs: [registro['seca'], registro['session_id']],
    );

    if (existing.isNotEmpty) {
      // Actualizar registro existente (misma balanza, misma sesión)
      await db.update(
        'registros_calibracion',
        registro,
        where: 'seca = ? AND session_id = ?', // ← ACTUALIZAR POR AMBOS CAMPOS
        whereArgs: [registro['seca'], registro['session_id']],
      );
      debugPrint('✅ Registro ACTUALIZADO - SECA: ${registro['seca']}, Session: ${registro['session_id']}');
    } else {
      // Insertar NUEVO registro (nueva balanza o nueva sesión)
  ...
  Future<Database> _initDatabase() async {
    ...
    if (!dbExists) {
      try {
        ...
        debugPrint('Error creando directorio: $e');
      }
  ...
  Future<void> _onCreate(Database db, int version) async {
    // Crear la tabla principal de calibración
    await db.execute('''
