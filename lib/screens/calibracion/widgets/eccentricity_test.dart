...
class _EccentricityTestState extends State<EccentricityTest> {
  ...
  final TextEditingController _oneThirdPmax1Controller = TextEditingController();
  ...
  final Map<String, List<String>> _platformOptions = {
    'Rectangular': [
      'Rectangular 3D', 'Rectangular 3I', 'Rectangular 3F', 'Rectangular 3A',
      'Rectangular 5D', 'Rectangular 5I', 'Rectangular 5F', 'Rectangular 5A'
    ],
    'Circular': [
      'Circular 5D', 'Circular 5I', 'Circular 5F', 'Circular 5A',
      'Circular 4D', 'Circular 4I', 'Circular 4F', 'Circular 4A'
    ],
    'Cuadrada': ['Cuadrada D', 'Cuadrada I', 'Cuadrada F', 'Cuadrada A'],
    'Triangular': ['Triangular I', 'Triangular F', 'Triangular A', 'Triangular D'],
  };
  ...
  Future<void> _saveDataToDatabase() async {
    ...
    for (int i = 0; i < _positionControllers.length; i++) {
      ...
      if (_indicationControllers[i].text.isEmpty) {
        _showSnackBar('Por favor ingrese la indicación ${i + 1}', isError: true);
        return;
      }
    ...
    try {
      final registro = {
        ...
        'posicion1': _positionControllers.isNotEmpty ? _positionControllers[0].text : '',
        'indicacion1': _indicationControllers.isNotEmpty ? _indicationControllers[0].text : '',
        'retorno1': _returnControllers.isNotEmpty ? _returnControllers[0].text : '',
        'posicion2': _positionControllers.length > 1 ? _positionControllers[1].text : '',
        'indicacion2': _indicationControllers.length > 1 ? _indicationControllers[1].text : '',
        'retorno2': _returnControllers.length > 1 ? _returnControllers[1].text : '',
        'posicion3': _positionControllers.length > 2 ? _positionControllers[2].text : '',
        'indicacion3': _indicationControllers.length > 2 ? _indicationControllers[2].text : '',
        'retorno3': _returnControllers.length > 2 ? _returnControllers[2].text : '',
        'posicion4': _positionControllers.length > 3 ? _positionControllers[3].text : '',
        'indicacion4': _indicationControllers.length > 3 ? _indicationControllers[3].text : '',
        'retorno4': _returnControllers.length > 3 ? _returnControllers[3].text : '',
        'posicion5': _positionControllers.length > 4 ? _positionControllers[4].text : '',
        'indicacion5': _indicationControllers.length > 4 ? _indicationControllers[4].text : '',
        'retorno5': _returnControllers.length > 4 ? _returnControllers[4].text : '',
        'posicion6': _positionControllers.length > 5 ? _positionControllers[5].text : '',
        'indicacion6': _indicationControllers.length > 5 ? _indicationControllers[5].text : '',
        'retorno6': _returnControllers.length > 5 ? _returnControllers[5].text : '',
                                          ...
                                          return PopupMenuButton<String>(
                                            ...
                                            itemBuilder: (BuildContext context) {
                                              final baseValue = double.tryParse(
                                                  _indicationControllers[index].text) ?? 0.0;
                                              List<String> options = [
                                                for (int i = 5; i >= 1; i--)
                                                  (baseValue + (i * d1)).toStringAsFixed(decimalPlaces),
                                                baseValue.toStringAsFixed(decimalPlaces),
                                                for (int i = 1; i <= 5; i++)
                                                  (baseValue - (i * d1)).toStringAsFixed(decimalPlaces),
                                              ];
                                              return options.map((value) => PopupMenuItem<String>(
                                                value: value,
                                                child: Text(value),
                                              )).toList();
                                            },
