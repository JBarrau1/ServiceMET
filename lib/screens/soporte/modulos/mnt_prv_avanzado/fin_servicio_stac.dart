  ...
  Future<void> _exportDataToCSV(BuildContext context) async {
    ...
    try {
      ...
      if (!tableNames.contains('inf_cliente_balanza') ||
          !tableNames.contains('mnt_prv_avanzado_stac')) {
      ...
      final List<Map<String, dynamic>> registros = await db.rawQuery('''
        ...
        LEFT JOIN mnt_prv_avanzado_stac AS mprs
      ...
      final fileName =
          '${widget.dbName}_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}mnt_prv_avanzado_stac.csv';
  ...
  Future<void> _copyDataFromId1() async {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        await db.insert('mnt_prv_avanzado_stac', {...data, 'id': nextId});

        final List<Map<String, dynamic>> tableInfo =
            await db.rawQuery('PRAGMA table_info(mnt_prv_avanzado_stac)');
