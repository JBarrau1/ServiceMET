...
class _FinServicioVinternasScreenState
  ...
  Future<void> _exportDataToCSV(BuildContext context) async {
    ...
    try {
      ...
      if (!tableNames.contains('inf_cliente_balanza') ||
          !tableNames.contains('verificaciones_internas')) {
        throw Exception(
            'Tablas básicas requeridas no encontradas en la base de datos');
      }

      // 3. Obtener datos combinados SOLO de las dos tablas requeridas
      final registros = await db.rawQuery('''
      SELECT icb.*, vi.*
      FROM inf_cliente_balanza AS icb
      LEFT JOIN verificaciones_internas AS vi ON icb.cod_metrica = vi.cod_metrica
    ...
    ''', [widget.codMetrica]);
      ...
      for (var registro in registrosDepurados) {
        ...
        for (var header in headers) {
          ...
          if (value is double) {
            row.add(value.toStringAsFixed(2));
      ...
      final fileName =
          '${widget.dbName}_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}_verificaciones_internas.csv';
  ...
  Future<void> _exportBackupCSV() async {
    ...
    try {
      ...
      // Obtener datos combinados SOLO de las dos tablas requeridas
      final registros = await db.rawQuery('''
      SELECT icb.*, vi.*
      FROM inf_cliente_balanza AS icb
      LEFT JOIN verificaciones_internas AS vi ON icb.cod_metrica = vi.cod_metrica
    ''');
      ...
      if (externalDir != null) {
        ...
        final fileName =
            '${widget.dbName}_verificaciones_internas_auto_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}.csv';
        final backupFile = File('${backupDir.path}/$fileName');
        await backupFile.writeAsBytes(csvBytes);
      }
    } catch (e) {
  ...
  Future<void> _copyDataFromId1() async {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        await db.insert('verificaciones_internas', {...data, 'id': nextId});

        final List<Map<String, dynamic>> tableInfo =
            await db.rawQuery('PRAGMA table_info(verificaciones_internas)');
