  ...
  Future<void> _exportDataToCSV(BuildContext context) async {
    ...
    try {
      ...
      if (!tableNames.contains('inf_cliente_balanza') ||
          !tableNames.contains('diagnostico')) {
      ...
      final fileName =
          '${widget.dbName}_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}diagnostico.csv';
  ...
  Future<void> _copyDataFromId1() async {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        await db.insert('diagnostico', {...data, 'id': nextId});

        final List<Map<String, dynamic>> tableInfo =
        await db.rawQuery('PRAGMA table_info(diagnostico)');
