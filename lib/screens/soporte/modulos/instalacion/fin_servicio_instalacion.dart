...
class FinServicioInstalacionScreen extends StatefulWidget {
  ...
  @override
  _FinServicioInstalacionScreenState createState() =>
      _FinServicioInstalacionScreenState();
}

class _FinServicioInstalacionScreenState
  ...
  Future<void> _exportDataToCSV(BuildContext context) async {
    ...
    try {
      ...
      if (!tableNames.contains('inf_cliente_balanza') ||
          !tableNames.contains('instalacion')) {
      ...
      final fileName =
          '${widget.dbName}_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}instalacion.csv';
  ...
  Future<void> _copyDataFromId1() async {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        await db.insert('instalacion', {...data, 'id': nextId});

        final List<Map<String, dynamic>> tableInfo =
            await db.rawQuery('PRAGMA table_info(instalacion)');
