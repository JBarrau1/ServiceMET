...
class FinServicioMntcorrectivoScreen extends StatefulWidget {
  ...
  const FinServicioMntcorrectivoScreen({
    super.key,
  ...
  @override
  _FinServicioMntcorrectivoScreenState createState() =>
      _FinServicioMntcorrectivoScreenState();
}

class _FinServicioMntcorrectivoScreenState
    extends State<FinServicioMntcorrectivoScreen> {
  String? errorMessage;
  ...
  Future<void> _exportDataToCSV(BuildContext context) async {
    ...
    try {
      ...
      if (!tableNames.contains('inf_cliente_balanza') || !tableNames.contains('mnt_correctivo')) {
        throw Exception('Tablas básicas requeridas no encontradas en la base de datos');
      }

      // 3. Verificar si la tabla diagnostico tiene datos
      bool diagnosticoHasData = false;
      if (tableNames.contains('diagnostico')) {
        final count = Sqflite.firstIntValue(await db.rawQuery(
            'SELECT COUNT(*) FROM diagnostico WHERE cod_metrica IS NOT NULL'
        )) ?? 0;
        diagnosticoHasData = count > 0;
      }

      // 4. Obtener datos combinados según disponibilidad
      List<Map<String, dynamic>> registros;

      if (diagnosticoHasData) {
        registros = await db.rawQuery('''
          SELECT icb.*, mc.*, d.*
          FROM inf_cliente_balanza AS icb
          LEFT JOIN mnt_correctivo AS mc ON icb.cod_metrica = mc.cod_metrica
          LEFT JOIN diagnostico AS d ON icb.cod_metrica = d.cod_metrica
      ...
      // 5. Depurar datos
      final List<Map<String, dynamic>> registrosDepurados = await _depurarDatos(registros);
      ...
      for (var registro in registrosDepurados) {
        ...
        for (var header in headers) {
          ...
          if (value is double) {
            row.add(value.toStringAsFixed(2)); // Formatear números con 2 decimales
      ...
      String csv = const ListToCsvConverter(
        fieldDelimiter: ';',
        textDelimiter: '"',
        eol: '\r\n', // Formato Windows para Excel
      ).convert(rows);
      final csvBytes = utf8.encode(csv);

      // 8. Guardar en directorio interno
      ...
      final fileName = '${widget.dbName}_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}_mnt_correctivo.csv';
      final internalFile = File('${csvDir.path}/$fileName');
      await internalFile.writeAsBytes(csvBytes);

      // 9. Permitir al usuario elegir ubicación adicional
      if (!mounted) return;
      Navigator.of(context).pop(); // Cerrar diálogo de carga
  ...
  Future<void> _exportBackupCSV() async {
    ...
    try {
      // Verificar si la tabla diagnostico tiene datos
      bool diagnosticoHasData = false;
      final tables = await db.rawQuery("SELECT name FROM sqlite_master WHERE type='table'");
      if (tables.any((t) => t['name'] == 'diagnostico')) {
        final count = Sqflite.firstIntValue(await db.rawQuery(
            'SELECT COUNT(*) FROM diagnostico WHERE cod_metrica IS NOT NULL'
        )) ?? 0;
        diagnosticoHasData = count > 0;
      ...
      // Obtener datos combinados según disponibilidad
      List<Map<String, dynamic>> registros;
      ...
      if (externalDir != null) {
        ...
        final fileName = '${widget.dbName}_mnt_correctivo_auto_${DateFormat('yyyy-MM-dd_HH-mm-ss').format(DateTime.now())}.csv';
        final backupFile = File('${backupDir.path}/$fileName');
        await backupFile.writeAsBytes(csvBytes);
      }
    } catch (e) {
      debugPrint('Error en backup CSV: $e');
  ...
  Future<void> _copyDataFromId1() async {
    ...
    try {
      ...
      if (result.isNotEmpty) {
        ...
        await db.insert('mnt_correctivo', {...data, 'id': nextId});

        final List<Map<String, dynamic>> tableInfo =
        await db.rawQuery('PRAGMA table_info(mnt_correctivo)');
