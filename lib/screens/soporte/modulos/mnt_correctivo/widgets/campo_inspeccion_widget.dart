...
class _CampoInspeccionWidgetState extends State<CampoInspeccionWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    final estado = widget.campo.estado;
                ...
                children: [
                  ...
                  TextField(
                    ...
                    controller: TextEditingController(
                        text: widget.campo.comentario ?? '')
                      ..selection = TextSelection.collapsed(
                        offset: widget.campo.comentario?.length ?? 0,
                      ),
                  ),
                  const SizedBox(height: 16),
  ...
  Widget _buildDropdownEstado() {
    ...
    return DropdownButtonFormField<String>(
      initialValue:
          widget.campo.estado.isEmpty ? '4 No aplica' : widget.campo.estado,
      decoration: InputDecoration(
        labelText: 'Estado',
        border: OutlineInputBorder(
          borderRadius: BorderRadius.circular(8),
        ...
        prefixIcon: Icon(
          _iconMap[widget.campo.estado] ?? Icons.help_outline,
          color: _colorMap[widget.campo.estado] ?? Colors.grey,
        ),
      ),
  ...
  Widget _buildDropdownSolucion() {
    ...
    return DropdownButtonFormField<String>(
      initialValue: (widget.campo.solucion?.isEmpty ?? true)
          ? 'No aplica'
          : widget.campo.solucion!,
      decoration: InputDecoration(
        labelText: 'Solución',
        border: OutlineInputBorder(
          borderRadius: BorderRadius.circular(8),
        ...
        prefixIcon: Icon(
          iconos[widget.campo.solucion] ?? Icons.help_outline,
          color: colores[widget.campo.solucion] ?? Colors.grey,
        ),
      ),
      items: opciones.map((String value) {
      ...
      onChanged: (newValue) {
        if (newValue != null) {
          setState(() {
            widget.campo.solucion = newValue;
            widget.onChanged();
          });
        }
      },
        ...
        children: [
          ...
          Positioned(
            ...
            child: GestureDetector(
              onTap: () {
                setState(() {
                  widget.campo.fotos.removeAt(index);
                  widget.onChanged();
                });
              },
              child: Container(
  ...
  Future<void> _tomarFoto() async {
    try {
      ...
      if (photo != null) {
        setState(() {
          widget.campo.fotos.add(File(photo.path));
          widget.onChanged();
        });
      }
    } catch (e) {
